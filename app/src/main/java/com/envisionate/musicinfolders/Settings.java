...
public class Settings extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            ((TextView)findViewById(R.id.build_date)).setText(String.format("Build date: %s",BuildConfig.BUILD_DATE));
        } catch ( Exception ex) {
        }
        ...
        if ( item.getItemId() == android.R.id.home ) {
            ...
            if ( ! ( null == CarEntitiesScreen.getWaitingScreen() ) ) {
                new Handler(getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
                        Intent intent = new Intent("com.envisionate.musicinfolders.SETUP_DONE");
